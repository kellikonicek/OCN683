...
```{r}
raw <- read_csv(here("week_1","data","foraging.height.edit.csv")) %>%
  filter(SPECIES != "OMAO") %>%
...
```{r}
# Model 
rat.species.model <- lm(foraging.ht.m ~ SPECIES*Rat_Removal, data = raw)
#Summary 
...
QQ plot is -- modest deviation 
Index plot- row in the dataframe- pattern is there because data happened to be sorted by foraging heights 

Residual vs. predicted-- we are looking for the amount of variation-- assuming the variation is a constant- for linear models, the spread is constant around 

sometimes it's better to plot with the predictor values resid_xpanel(mod)
...
Anova
```
There's no evidence the effect of removing rats differs between the different bird species 

Species AND rat removal explain a lot of variation 
...
since the interaciton isn't significant, you can take the interactio OUT of the model and plot the predictors one at a time 
...
ggplot(birds.sub, aes(tot.arth.bm, foraging.ht.m, col= rat_removal))+ geomn_point+xcale_x_log10_+facet_wrap_geom_smooth(methol=lm)

seems to be a stronger relationship between area and foraging height than biomass and foraging height
...
kyle model= lm(foraging.ht.m~SPECIES+Rat remove* log area+log.arthropods*Rat_removal, data = birds.sub
...
ADD IN DATE KYLE
klyle= lm(foraging.ht.m ~ SPECIES + Rat_removal+log.area+log.arthropods*Rat_moval+DATE)


But the date in as a continous predictor -- you're assuming foraging height is going up or going down 

if the goal is to test for a temporal trend, do as continuous 
...
other thing you'd want to incorporate- random effects to deal with repeated measures 

experimental unit is the kipuka-- went back multiple times tothese 
pseudoreplication if you're not tellig the model they are the same 
use date as a random effect instead of fixed effect, random effect for observer 

you still have affect of arthropods, area 

LMER- need to use for random 
LM does not do random 


when you add in the random effect, the results become more varied again because you are properly accounting for the degrees of freedom with random/fixed effects 